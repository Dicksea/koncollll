...
console.log(color('[DEV]', 'cyan'), color('Ariel SP', 'magenta'))
    let { version, isLatest } = await fetchLatestBaileysVersion()
    const mans = mansConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Ariel Sandy Permana', 'Aloha', '5.4'],
        auth: state,
        version
    })
...
require("./Ariel-SP_Ganteng")(mans, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
mans.ev.on('groups.update', async pea => {
    ...
    // Get Profile Picture Group
       ...
       ppgc = 'https://telegra.ph/file/7a9b4168c6b7e28bf375b.jpg'
       }
...
//
WCome = await getBuffer(`https://hardianto.xyz/api/tools/welcomer?nama=${encodeURIComponent(nama)}&namaGb=${encodeURIComponent(metadata.subject)}&pepeGb=${encodeURIComponent(ppgroup)}&totalMem=${encodeURIComponent(memb)}&pepeUser=${encodeURIComponent(ppuser)}&bege=https://telegra.ph/file/7a9b4168c6b7e28bf375b.jpg&apikey=hardianto`)

GBye = await getBuffer(`https://hardianto.xyz/api/tools/leave?nama=${encodeURIComponent(nama)}&namaGb=${encodeURIComponent(metadata.subject)}&pepeGb=${encodeURIComponent(ppgroup)}&totalMem=${encodeURIComponent(memb)}&pepeUser=${encodeURIComponent(ppuser)}&bege=https://telegra.ph/file/7a9b4168c6b7e28bf375b.jpg&apikey=hardianto`)
                if (anu.action == 'add') {
                    mans.sendMessage(anu.id, { image: { url: `https://telegra.ph/file/7a9b4168c6b7e28bf375b.jpg` }, contextInfo: { mentionedJid: [num] }, caption: `Hi @${num.split("@")[0]}!!` })
                } else if (anu.action == 'remove') {
                    mans.sendMessage(anu.id, { image: { url: `https://telegra.ph/file/7a9b4168c6b7e28bf375b.jpg` }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Keluar Dari Group ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
